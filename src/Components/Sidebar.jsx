...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  const location = useLocation();
  const approvedFor = useSelector((state) => state.auth?.user?.reporter?.ApprovedFor);

  const navItems = [
    {
      to: "/dashboard",
      label: "Dashboard",
      icon: <BiGridAlt className="text-xl" />,
      visible: approvedFor === "News" || approvedFor === "Both",
    },
    {
      to: "/news",
      label: "Add News",
    ...
    },
    {
      ...
      visible: approvedFor === "Blogs" || approvedFor === "Both",
    },
  ];
